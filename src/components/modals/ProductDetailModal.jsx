...
export default function ProductDetailModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 flex h-[780px] w-[min(1100px,92vw)] flex-col overflow-hidden rounded-[20px] bg-white shadow-[0_30px_80px_rgba(0,0,0,0.25)]">
        <header className="grid grid-cols-[auto_1fr_auto] items-center border-b border-stone-200 bg-white px-5 py-3.5">
          <button
        ...
        <div className="flex-1 overflow-auto">
          <div className="grid min-h-[420px] lg:grid-cols-[1.1fr_0.9fr]">
            <div className="relative flex items-center justify-center bg-[#f5f6f8]">
              {detailLoading ? (
                <div className="h-full min-h-[420px] w-full animate-pulse bg-gradient-to-r from-stone-100 via-stone-50 to-stone-100" />
              ) : activeImages?.[0] ? (
                <img
                  src={activeImages[0]}
                ...
                <div className="flex h-full min-h-[420px] w-full items-center justify-center text-sm text-stone-500">
                  暂无图片
                </div>
              )}
              {isOutOfStock && (
                <span className="absolute right-4 top-4 rounded-full bg-stone-900/90 px-3 py-1.5 text-xs font-medium text-white">
                  售罄
                </span>
              )}
            </div>
            ...
            <div className="flex flex-col gap-4 p-[22px]">
              <div className="space-y-4">
                {detailBrand && (
                  <div className="inline-flex self-start rounded-full bg-stone-900 px-3 py-1 text-xs text-white">
                    {detailBrand}
                  </div>
                )}
              ...
              <div className="space-y-2">
                ...
                <div className="grid gap-3">
                  {detailSpecList.length ? (
                    ...
                    <div className="rounded-[14px] bg-stone-100 px-3.5 py-2.5 text-sm text-stone-500">
                      暂无规格
                    </div>
                  )}
                </div>
              ...
              <div className="flex items-center justify-between text-sm text-stone-500">
                <span>购买数量</span>
                <div className="inline-flex items-center gap-3 rounded-full border border-stone-200 bg-white px-3 py-2">
                  <button
                    type="button"
        ...
        <footer className="shrink-0 border-t border-stone-200 bg-white px-5 py-4">
          <div className="flex justify-end gap-3">
            <button
              className="rounded-full border border-stone-900 bg-white px-5 py-2.5 text-sm font-semibold text-stone-900 disabled:cursor-not-allowed disabled:opacity-50"
              type="button"
